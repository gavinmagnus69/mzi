class GOST3411:
    ...
    # X transformation (bitwise XOR)
    @staticmethod
    def xor(a, b):
        return [a[i] ^ b[i] for i in range(64)]

    # S transformation - substitution using the PI table 
    @staticmethod
    def S(a):
        return [GOST3411.Pi[v] for v in reversed(a)]

    # P transformation - permute bytes using table t
    @staticmethod
    def P(a):
        return [a[elem] for elem in reversed(GOST3411.t)]

    # L transformation - multiply the vector by the linear matrix
    @staticmethod
    def L(a):
        result = [0] * 64
    ...
    # For use by the compression function
    @staticmethod
    def E(K, m):
